...
class _ExpensesManagementScreenState extends State<ExpensesManagementScreen> {
  ...
  void _onScroll() {
    ...
    if (maxScroll > 0 && currentScroll >= maxScroll * 0.9) {
      ...
      final provider = Provider.of<ExpenseProvider>(context, listen: false);
      if (provider.hasMore && !provider.isLoading && provider.expenses.isNotEmpty) {
  ...
  void _showReviewDialog(Expense expense, ExpenseStatus status) {
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              await _reviewExpense(expense, status, noteController.text.trim().isEmpty ? null : noteController.text.trim());
            },
  ...
  Future<void> _reviewExpense(Expense expense, ExpenseStatus status, String? note) async {
    final provider = Provider.of<ExpenseProvider>(context, listen: false);
    
    final success = await provider.reviewExpense(expense.id, status, note);
    ...
    if (mounted) {
      if (success) {
        ...
        _showWhatsAppOption(expense, status, note);
      } else {
  ...
  Future<void> _showWhatsAppOption(Expense expense, ExpenseStatus status, String? note) async {
    ...
    if (shouldSend == true && mounted) {
      await _sendExpenseWhatsApp(expense, status, note);
    }
  }

  Future<void> _sendExpenseWhatsApp(Expense expense, ExpenseStatus status, String? note) async {
    try {
      // Fetch employee to get phone number
      final employeeService = EmployeeService();
      final employee = await employeeService.getEmployeeById(expense.employeeId);
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${expense.employeeIdStr} • ${expense.designation ?? "N/A"}',
                          style: TextStyle(
                            fontSize: 12,
                  ...
                  if (expense.status == ExpenseStatus.PENDING) ...[
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => _showReviewDialog(expense, ExpenseStatus.REJECTED),
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () => _showReviewDialog(expense, ExpenseStatus.APPROVED),
                    ...
                    const SizedBox(width: 8),
                    IconButton(
                      onPressed: () => _sendExpenseWhatsAppDirect(expense),
  ...
  TableRow _buildExpenseTableRow(Expense expense, int index) {
    Color statusColor;
    IconData statusIcon;
    ...
    return TableRow(
      ...
      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(
                                '${expense.employeeIdStr} • ${expense.designation ?? "N/A"}${expense.department != null ? ' • ${expense.department}' : ''}',
                                style: TextStyle(
                                  fontSize: 12,
                                  fontWeight: FontWeight.w500,
                                  color: Theme.of(context).colorScheme.onSurfaceVariant,
        ...
        TableCell(
          ...
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: _buildExpenseTableActionButtons(expense),
          ),
        ),
      ],
    );
  ...
  Widget _buildExpenseTableActionButtons(Expense expense) {
    if (expense.status == ExpenseStatus.PENDING) {
      return Row(
        mainAxisSize: MainAxisSize.min,
        children: [
      ...
      return Row(
        ...
        children: [
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              if (value == 'whatsapp') {
                _sendExpenseWhatsAppDirect(expense);
              }
            },
            itemBuilder: (context) => [
              const PopupMenuItem(
  ...
  Future<void> _sendExpenseWhatsAppDirect(Expense expense) async {
    try {
      // Fetch employee to get phone number
      final employeeService = EmployeeService();
      final employee = await employeeService.getEmployeeById(expense.employeeId);
