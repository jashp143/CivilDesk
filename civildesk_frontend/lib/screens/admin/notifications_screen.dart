...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _handleMarkAllAsRead() async {
    try {
      ...
      if (mounted) {
        Toast.error(context, 'Failed to mark all as read');
      }
    }
  }
    ...
    if (confirmed == true) {
      ...
      if (mounted) {
        if (success) {
          ...
          Toast.error(context, 'Failed to delete notification');
        }
      }
    }
  }
  ...
  void _handleNotificationTap(NotificationModel notification) {
    ...
    // Navigate based on notification type - simplified navigation
    if (type == 'TASK_ASSIGNED' || type == 'TASK_STATUS_CHANGED') {
      Navigator.pushNamed(context, AppRoutes.adminTasks);
    } else if (type.contains('LEAVE')) {
      Navigator.pushNamed(context, AppRoutes.adminLeave);
    } else if (type.contains('EXPENSE')) {
      Navigator.pushNamed(context, AppRoutes.adminExpenses);
    } else if (type.contains('OVERTIME')) {
      Navigator.pushNamed(context, AppRoutes.adminOvertime);
    } else if (type == 'FINALIZED_SALARY_SLIPS') {
      Navigator.pushNamed(context, AppRoutes.adminSalarySlips);
    } else if (type == 'BROADCAST_MESSAGE') {
      Navigator.pushNamed(context, AppRoutes.adminBroadcasts);
    }
  }
